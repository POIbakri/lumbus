...
export default function DashboardPage() {
  ...
  const loadOrders = async () => {
    try {
      ...
      setOrders(data as OrderWithPlan[]);
    } catch (error) {
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          {referralStats && (
            <div className="mb-12 animate-slide-up" style={{animationDelay: '0.2s'}}>
              <h2 className="heading-md mb-6">REFER & EARN</h2>
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    ...
                    <div>
                      <div className="mb-6">
                        ...
                        <h3 className="heading-sm mb-3">SHARE & GET REWARDED</h3>
                    ...
                    {/* Right: Stats Section */}
                    <div>
                      <h3 className="heading-sm mb-4">YOUR STATS</h3>
                      <div className="space-y-4">
                        <div className="bg-white rounded-xl p-6">
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-black uppercase text-sm text-muted-foreground">
                              Total Clicks
                            </span>
                        ...
                        {referralStats.pending_rewards > 0 && (
                          <div className="bg-yellow rounded-xl p-6 border-2 border-secondary">
                            <div className="flex justify-between items-center mb-2">
          ...
          <div className="mb-12">
            <div className="flex justify-between items-center mb-6">
              <h2 className="heading-md">ACTIVE ESIMS</h2>
              <Link href="/plans">
                <Button className="btn-lumbus bg-foreground text-white hover:bg-foreground/90 font-black touch-ripple elastic-bounce pulse-glow">
                  + BUY NEW ESIM
                </Button>
              </Link>
            </div>
            ...
            {activeOrders.length === 0 ? (
              <Card className="bg-purple border-2 border-accent shadow-lg animate-slide-up">
                <CardContent className="pt-6 text-center py-12">
                  ...
                  <h3 className="font-black text-2xl mb-2">NO ACTIVE ESIMS</h3>
                  <p className="font-bold text-muted-foreground mb-6">
                    Get started by purchasing your first eSIM
                  </p>
                  <Link href="/plans">
                    <Button className="btn-lumbus bg-foreground text-white hover:bg-foreground/90 font-black text-lg px-8 py-6">
                      BROWSE PLANS
                    </Button>
                  </Link>
                </CardContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {activeOrders.map((order, index) => {
                  ...
                  // Mock data usage (in production, fetch from real API)
                  const dataUsed = Math.random() * (order.plan?.data_gb || 5);
                  const dataPercentage = getDataPercentage(dataUsed, order.plan?.data_gb || 5);
                    ...
                    >
                      ...
                      <CardHeader className="relative z-10">
                        <div className="flex justify-between items-start mb-2">
                          <Badge className={`${getStatusColor(order.status)} font-black uppercase text-xs px-3 py-1`}>
                            {order.status}
                          </Badge>
                          <div className="text-right">
                            <div className="text-2xl font-black">{order.plan?.region_code}</div>
                          </div>
                        </div>
                        <CardTitle className="text-2xl font-black uppercase">{order.plan?.name}</CardTitle>
                      </CardHeader>
                      ...
                      <CardContent className="space-y-4 relative z-10">
                        ...
                        <div className="p-4 bg-white rounded-xl">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="font-black text-sm">{dataUsed.toFixed(1)} / {order.plan?.data_gb} GB</span>
                          </div>
                        ...
                        <div className="p-4 bg-white rounded-xl">
                          <div className="flex justify-between items-center">
                            ...
                            <span className={`font-black text-2xl ${daysRemaining <= 5 ? 'text-destructive' : ''}`}>
                              {daysRemaining}
                            </span>
                          </div>
                          {daysRemaining <= 5 && (
                        ...
                        <div className="flex gap-3">
                          <Link href={`/install/${order.id}`} className="flex-1">
                            <Button className="w-full btn-lumbus bg-foreground text-white hover:bg-foreground/90 font-black text-sm py-4 touch-ripple">
                              VIEW DETAILS
                            </Button>
                          </Link>
                        </div>
                    ...
                    {pastOrders.map((order) => (
                      ...
                      >
                        <div className="flex-1">
                          <div className="font-black text-lg mb-1">{order.plan?.name}</div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {order.status === 'completed' && (
                            <Link href={`/install/${order.id}`}>
                              <Button className="btn-lumbus bg-foreground text-white hover:bg-foreground/90 font-black text-sm px-4 py-2">
                                VIEW
                              </Button>
                            </Link>
                          )}
